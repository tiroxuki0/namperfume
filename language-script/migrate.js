...
const getAllSubLang = () => {
  ...
  console.log(chalk.cyan("filenames", filenames, "filenamesMapping", filenames.map((fn) => path.join(folderPath, fn))))
...
const overwriteTranslationFiles = (defaultLanguageFolderPath, language) => {
  for (const folder of getAllSubLang(language)) {
    ...
    execSync(
      `rsync -aP --exclude=values.js /${defaultLanguageFolderPath}/ /${folder}/`
    );
...
const compareAndUpdateValueFile = async (language, defaultValues) => {
  ...
  if(addCount.length > 0){
    ...
    const missingPath = path.resolve("./language-script/missing", `${language}-${Date.now().toString()}.txt`);
...
export const migrate = async () => {
  try {
    const defaultLanguageFolderPath = path.resolve(
      "./locales",
      "eng"
    );
